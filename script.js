...
if(navLinks&&!navLinks.querySelector('a[href="practice.html"]')){
    const li=document.createElement('li');
    li.innerHTML='<a href="practice.html">Практика нәтижесі</a>';
    navLinks.appendChild(li);
}
if(navLinks){
    const currentPage=(window.location.pathname.split('/').pop()||'index.html').toLowerCase();
    navLinks.querySelectorAll('a').forEach(a=>a.classList.remove('active'));
    const activeLink=navLinks.querySelector(`a[href="${currentPage}"]`)||navLinks.querySelector('a[href="index.html"]');
