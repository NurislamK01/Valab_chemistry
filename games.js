...
function initBuild(){
    ...
    const decoyAtoms=['N','H','O','P','Cl','S'];
    const requiredAtoms=[...target.atoms];
    const extraCount=Math.max(3,6-target.atoms.length);
    const pool=[...requiredAtoms];
    while(pool.length<Math.min(10,target.atoms.length+extraCount)){
        pool.push(decoyAtoms[Math.floor(Math.random()*decoyAtoms.length)]);
    }
